...
static int
raid1_submit_nvme_read_request(struct raid_bdev_io *raid_io)
{
...
static int
raid1_submit_nvme_write_request(struct raid_bdev_io *raid_io)
{
	struct raid_bdev *raid_bdev = raid_io->raid_bdev;
	struct spdk_bdev_io *bdev_io = spdk_bdev_io_from_ctx(raid_io);
    ...
    // printf("Got write on slba %lu nlb %lu\n", nvme_lba, nvme_nlb);
...
raid1_submit_rw_request(struct raid_bdev_io *raid_io)
{
	...
	case SPDK_BDEV_IO_TYPE_NVME_IO:
        opc = bdev_io->u.nvme_passthru.cmd.opc;
        if (opc == SPDK_NVME_OPC_WRITE) {
            ret = raid1_submit_nvme_write_request(raid_io);
        } else if (opc == SPDK_NVME_OPC_READ) {
            ret = raid1_submit_nvme_read_request(raid_io);
        } else {
            ret = -EINVAL;
        }
		break;
