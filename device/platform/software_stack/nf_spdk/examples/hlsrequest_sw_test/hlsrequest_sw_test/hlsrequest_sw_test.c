...
struct test_context {
  ...
  uint64_t start_time;
  uint64_t req_end_time[4];
  void* req_addr[4];
  int request_exec_count;
  int test5_fin_count;
} ctx;
...
void hlsacccompute_req_callback(struct spdk_hlsacccompute_request *request,
  ...
  if(request==ctx.req_addr[0]){
    SPDK_NOTICELOG("REQUEST 1 FINISHED\n");
    ...
    //}
    //for(int i=0;i<2;i++){
    //  request->tx_channel[0]->channel_poller(request->tx_channel[0]);
    //  request->rx_channel[0]->channel_poller(request->rx_channel[0]);
    //}
    //spdk_hlsacccompute_run_request(request->dev,ctx.req_addr[2],true);
    //SPDK_NOTICELOG("SECOND PREEMPT");
  }else if(request==ctx.req_addr[1]){
    SPDK_NOTICELOG("REQUEST 2 FINISHED\n");
    spdk_hlsacccompute_free_request(request->dev,request,true);
...
void hlsacccompute_request_inject(void* ctx){
...
static void compute_start(void *arg) {
  ...
  do {
    ...
    void* addr = spdk_zmalloc(PAGE_SIZE*1024*16,PAGE_SIZE,NULL,SPDK_ENV_SOCKET_ID_ANY,SPDK_MALLOC_SHARE);
    ...
    px2->program_id = 3;
    px2->apply_operators_id_map[0] = 1;
    ...
    for(int i=0;i<1;i++){
        //struct spdk_hlsacccompute_request* req = i==0?spdk_hlsacccompute_create_request(ctx.dev,px1):spdk_hlsacccompute_create_request(ctx.dev,px2);
        ...
        struct spdk_hlsacccompute_virtual_object *rx_ob =
          spdk_malloc(sizeof(struct spdk_hlsacccompute_virtual_object), 2,
                      NULL, SPDK_ENV_SOCKET_ID_ANY, SPDK_MALLOC_SHARE);
        int rx_len = i==0?(256*256+1):17;
        int tx_len = i==0?(256*256):16;
