UBOOT_SRC := software/uboot
UBOOT_ARCH := arm
UBOOT_LOC := $(abspath ./software/$(UBOOT_ARCH)-uboot)
...
UBOOT_DTB_LOC := $(UBOOT_LOC)/../../$(TARGET_LOC)/$(TARGET_PRJ)/ready_for_download/$(FPGA_BD)
UBOOT_DTB := $(UBOOT_DTB_LOC)/zynqmp.dtb
...
UBOOT_SCRIPT_FLAGS := -A arm -O linux -T script \
	-C none -a 0 -e 0 -n 'Custom BOOT config' \

UBOOT_SCRIPT := $(MULTI_RP_LOC)/boot/boot.script
...
ifneq ($(wildcard $(UBOOT_APP)),)
	@cp $(UBOOT_APP) $(UBOOT_DTB_LOC)/
endif
ifneq ($(wildcard $(UBOOT_SCRIPT)),)
	$(UBOOT_MKIMG) $(UBOOT_SCR_FLAGS) \
		-d $(abspath $(UBOOT_SCR)) \
		$(UBOOT_DTB_LOC)/boot.scr.uimg
endif
