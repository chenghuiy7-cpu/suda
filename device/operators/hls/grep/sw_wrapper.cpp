...
extern "C" int run(int thread_id) {
    // 运行主处理函数
    //printf("run\n");
    SimpleGrep(g_data_in[thread_id][0], g_data_out[thread_id][0],g_context[thread_id][0]);
...
extern "C" int  get_data_size(unsigned int port_id,int thread_id){
    return g_data_out[thread_id][port_id].size();
}
