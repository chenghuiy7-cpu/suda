...
int intr_setup(struct xlnx_dma_dev *xdev)
{
	...
	affd.pre_vectors++;
#endif
	xdev->num_vecs = min_t(int, num_vecs, num_online_cpus() + affd.pre_vectors);

	xdev->num_vecs = min_t(int, xdev->num_vecs, xdev->conf.msix_qvec_max);
	...
	rv = pci_alloc_irq_vectors_affinity(xdev->conf.pdev, xdev->num_vecs, xdev->num_vecs, PCI_IRQ_MSIX | PCI_IRQ_AFFINITY, &affd);
	if (rv < 0) {
		pr_err("Error enabling MSI-X (%d)\n", rv);
		goto free_intr_info;
	}
	for (i = 0; i < xdev->num_vecs; i++) {
		xdev->msix[i].vector = pci_irq_vector(xdev->conf.pdev, i);
	}
#else
