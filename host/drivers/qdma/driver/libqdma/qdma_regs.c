...
EXPORT_SYMBOL(qdma_device_read_config_register);
...
EXPORT_SYMBOL(qdma_device_write_config_register);
...
int qdma_get_pfch_tag(unsigned long dev_hndl, uint16_t qid, uint16_t *tag)
{
	...
	qdma_mbox_compose_pfch_tag_get(xdev->func_id, qid, m->raw);

	rv = qdma_mbox_msg_send(xdev, m, 1, QDMA_MBOX_MSG_TIMEOUT_MS);
	if (rv < 0)
		goto free_msg;

	rv = qdma_mbox_pfch_tag_get(m->raw, tag);
	if (rv < 0) {
		pr_err("Failed to get pfch tag, rv = %d", rv);
		rv = -EINVAL;
	}
...
EXPORT_SYMBOL(qdma_get_pfch_tag);
