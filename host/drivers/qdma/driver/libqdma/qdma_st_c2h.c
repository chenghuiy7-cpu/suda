...
int descq_flq_alloc_resource(struct qdma_descq *descq)
{
	...
	if (descq->conf.desc_bypass) {
		pr_debug("Set flq desc addr to its idx for C2H bypass q %u\n", descq->conf.qidx);
		for (i = 0; i < flq->size; i++, desc++) {
			desc->dst_addr = (u64)i;
		}
			...
			bool update_pidx, bool refill)
{
	...
	pr_debug("cb %p read, sgl %p", cb, req->sgl);
	...
	while ((i < fsgcnt) && tsg) {
		...
		while (flen && tsg) {
			...
			pr_debug("copy: %p(%u) -> %lx\n", faddr, copy, (uintptr_t)page_address(tsg->pg) + toff);
...
static void cmpt_next_n(struct qdma_descq *descq, unsigned int n)
{
	u8 *desc_cmpt_cur;

	if (descq->cidx_cmpt + n >= descq->conf.rngsz_cmpt) {
		descq->color ^= 1;
	}

	descq->cidx_cmpt = (descq->cidx_cmpt + n) % descq->conf.rngsz_cmpt;
	desc_cmpt_cur = (u8 *)descq->desc_cmpt_cur + descq->cidx_cmpt * descq->cmpt_entry_len;
	descq->desc_cmpt_cur = desc_cmpt_cur;
}
...
static inline u16 descq_rsp_len(struct qdma_descq *descq, __be64 cmpt)
{
	pr_debug("descq->conf.qidx = %d\n", descq->conf.qidx);
	if (descq->conf.qidx % 32 == 0) {
		return 64 + 4096;
	} else if (descq->conf.qidx % 32 <= 8) {
		return 16;
	} else {
		return 512;
	}
	// return descq->conf.qidx < 16 ? (64 + 4096) : 16;
	// return (cmpt >> 16) & 0xFFFF;
}
...
int parse_cmpl_entry(struct qdma_descq *descq, struct qdma_ul_cmpt_info *cmpl)
{
...
#if 0
	print_hex_dump(KERN_INFO, "parse cmpl entry ", DUMP_PREFIX_OFFSET,
	...
	cmpl->entry = cmpt;
	cmpl->f.format = 0;
	// C2H_CMPT_FORMAT_0 is 0x20001, so color bit is 1, err bit is 2
	...
	if (!cmpl->f.format && cmpl->f.desc_used) {
		...
		cmpl->len = descq_rsp_len(descq, cmpt[0]);
		// cmpl->len = *(u16 *)cmpl->entry;
		// cmpl->len = 512;
	} else
		cmpl->len = 0;
...
static int descq_process_completion_st_c2h_no_flq(struct qdma_descq *descq)
{
	...
	// Update pidx
	descq->pidx = ring_idx_incr(descq->pidx, pend, descq->conf.rngsz);
	// descq->pidx = ring_idx_incr(descq->pidx, 1, descq->conf.rngsz);
	descq->pidx_info.pidx = descq->pidx;

	pr_debug("%s: Update pidx to %u\n", descq->conf.name, descq->pidx);
	rv = queue_pidx_update(descq->xdev,
			descq->conf.qidx,
			descq->conf.q_type,
			&descq->pidx_info);
	if (unlikely(rv < 0))  {
					...
					bool upd_cmpl)
{
	...
	if (qconf->desc_bypass) {
		return descq_process_completion_st_c2h_no_flq(descq);
	}
	...
	while (likely(proc_cnt < budget)) {
		...
		pr_debug("New cmpl entry len %u\n", cmpl.len);

#if 0
		// print_hex_dump(KERN_INFO, "Cmpl entry content: ", DUMP_PREFIX_OFFSET,
		// 		16, 1, (void *)&cmpl, descq->cmpt_entry_len,
		// 		false);
#endif
	...
	if (proc_cnt) {
		...
		if (flq->pidx_pend != pidx_pend) {
			pr_debug("flp->pidx_pend %u pidx_pend %u\n", flq->pidx_pend, pidx_pend);
