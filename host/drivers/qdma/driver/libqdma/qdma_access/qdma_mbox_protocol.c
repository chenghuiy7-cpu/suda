...
enum mbox_msg_op {
	...
	/** @MBOX_OP_Q_PFCH_TAG_GET: Get C2H prefetch tag for queue */
	MBOX_OP_Q_PFCH_TAG_GET,
	 ...
	 */
	MBOX_OP_PF_RESET_VF_BYE_RESP,
	/** @MBOX_OP_Q_PFCH_TAG_GET_RESP: response to @MBOX_OP_Q_PFCH_TAG_GET */
	MBOX_OP_Q_PFCH_TAG_GET_RESP,
...
union qdma_mbox_txrx {
		...
		/** Prefetch tag get message */
		struct mbox_msg_pfch_tag_get pfch_tag_get;
				 ...
				 uint32_t *resp_msg)
{
	...
	break;
	case MBOX_OP_Q_PFCH_TAG_GET:
	{
		struct mbox_msg_pfch_tag_get *ptg_cfg = &rcv->pfch_tag_get;
		uint32_t qid = ptg_cfg->byp_qid + (hdr->src_func_id - 4) * 32;
		uint32_t val, read_qid;
		pr_warn("Got pfch tag get request for VF %u qid %u (%u)\n", hdr->src_func_id, qid, ptg_cfg->byp_qid);
		qdma_reg_write(dev_hndl, 0x1408, qid);

		do {
			val = qdma_reg_read(dev_hndl, 0x140C);
			read_qid = (val >> 8) & 0xFFF;
		} while (read_qid != qid);

		ptg_cfg->byp_tag = val & 0x7F;

		pr_warn("Got pfch tag get response for VF %u qid %u (%u) tag %u\n", hdr->src_func_id, qid, ptg_cfg->byp_qid, ptg_cfg->byp_tag);
	...
	case MBOX_OP_QCTXT_RD:
	{
		...
		rv = mbox_read_queue_contexts(dev_hndl, qctxt->qid_hw,
						...
						qctxt->c2h,
						qctxt->cmpt_ctxt_type,
						(struct qdma_descq_context *)&resp->qctxt.descq_conf);
	}
	break;
...
int qdma_mbox_compose_pfch_tag_get(uint16_t func_id,
				 uint16_t qid, uint32_t *raw_data)
{
	union qdma_mbox_txrx *msg = (union qdma_mbox_txrx *)raw_data;
	...
	qdma_mbox_memset(raw_data, 0, sizeof(union qdma_mbox_txrx));
	msg->hdr.op = MBOX_OP_Q_PFCH_TAG_GET;
	msg->hdr.src_func_id = func_id;
	msg->pfch_tag_get.byp_qid = qid;
...
int qdma_mbox_pfch_tag_get(uint32_t *rcv_data, uint16_t *tag)
{
	union qdma_mbox_txrx *msg = (union qdma_mbox_txrx *)rcv_data;

	*tag = msg->pfch_tag_get.byp_tag;
			     ...
			     struct qdma_descq_context *ctxt)
{
	...
	qdma_mbox_memcpy(ctxt, &msg->qctxt.descq_conf,
