			...
			do { /* to support zero byte transfer */
				...
				// if (descq->conf.desc_bypass) {
					if (i == 0) {
						...
						pr_debug("Add SOP for desc %u\n", desc->len);
					}

					if ((i == sg_max - 1)) {
						desc->flags |= S_H2C_DESC_F_EOP;
						pr_debug("Add EOP for desc %u\n", desc->len);
					}
				// }
...
static void desc_alloc_irq(struct qdma_descq *descq)
{
	...
	if (xdev->conf.qdma_drv_mode == DIRECT_INTR_MODE) {
		...
		/*
		 * Pick interrupt vector with core affinity
		 * 
		 ...
		 * Data QE: 9-16
		 */
		pr_debug("dvec_start_idx %u qidx %u num_vecs %u\n",
			xdev->dvec_start_idx, descq->conf.qidx, xdev->num_vecs);
		if (descq->conf.qidx <= 8) {
			// Admin or QE queues
			idx = xdev->dvec_start_idx + descq->conf.qidx % (xdev->num_vecs - xdev->dvec_start_idx);
		} else {
			// Data queues
			idx = xdev->dvec_start_idx + (descq->conf.qidx - 8) % (xdev->num_vecs - xdev->dvec_start_idx);
		}
	}
	descq->intr_id = idx;
...
int qdma_descq_alloc_resource(struct qdma_descq *descq)
{
	...
	if (descq->conf.st && (descq->conf.q_type == Q_C2H)) {
		...
		pr_debug("%s: rngsz = %u, buf_pg_shift = %d, buf_pg_mask = %d, desc_buf_size = %d, desc_pg_shift = %d, flq->desc_pg_order = %d, flq->max_pg_offset = %d",
				descq->conf.name,
				descq->conf.rngsz,
				flq->buf_pg_shift,
				flq->buf_pg_mask,
...
void qdma_descq_config(struct qdma_descq *descq, struct qdma_queue_conf *qconf,
		 int reconfig)
{
	pr_debug("qconf: %s-%s-%u, q_type %u, pipe %u, irq_en %u, desc_rnd_sz_idx %u, \
wb_status_en: %u, cmpl_status_acc_en: %u, cmpl_status_pend_chk: %u, desc_bypass: %u, pfetch_en: %u, fetch_credit: %u, \
st_pkt_mode: %u, c2h_buf_sz_idx: %u, cmpl_rng_sz_idx: %u, cmpl_desc_sz: %u, cmpl_stat_en: %u, cmpl_udd_en: %u, cmpl_timer_idx: %d \
cmpl_cnt_th_idx: %u, cmpl_trig_mode: %u, cmpl_en_intr: %u, sw_desc_sz: %u, pfetch_bypass: %u, cmpl_ovf_chk_dis: %u, \
port_id: %u, at: %u, adaptive_rx: %u, latency_optimize: %u, init_pidx_dis: %u, mm_channel: %u, quld: %lX \
pidx_acc: %u, rngsz: %u, rngsz_cmpt: %u, c2h_bufsz: %u, ping_pong_en: %u, aperture_size: %u\n",
			  qconf->name, qconf->st ? "ST" : "MM", qconf->qidx,
			  qconf->q_type, qconf->pipe, qconf->irq_en, qconf->desc_rng_sz_idx, qconf->wb_status_en,
			  qconf->cmpl_status_acc_en, qconf->cmpl_status_pend_chk, qconf->desc_bypass, qconf->pfetch_en, qconf->fetch_credit, 
			  qconf->st_pkt_mode, qconf->c2h_buf_sz_idx, qconf->cmpl_rng_sz_idx, qconf->cmpl_desc_sz, qconf->cmpl_stat_en, 
			  qconf->cmpl_udd_en, qconf->cmpl_timer_idx, qconf->cmpl_cnt_th_idx, qconf->cmpl_trig_mode, qconf->cmpl_en_intr, qconf->sw_desc_sz, 
			  qconf->pfetch_bypass, qconf->cmpl_ovf_chk_dis, qconf->port_id, qconf->at, qconf->adaptive_rx, qconf->latency_optimize, 
			  qconf->init_pidx_dis, qconf->mm_channel, qconf->quld, qconf->pidx_acc, qconf->rngsz, qconf->rngsz_cmpt, qconf->c2h_bufsz, 
			  qconf->ping_pong_en, qconf->aperture_size);
	if (!reconfig) {
		int len;
		...
		descq->conf.port_id = qconf->port_id;
	}
}
...
{
	struct qdma_request *req = (struct qdma_request *)cb;
	uint64_t *first = (uint64_t *)((uintptr_t)page_to_virt(req->sgl->pg) + req->sgl->offset);
	pr_debug("First 64b is %llX\n", *first);
			...
			int end, char *buf, int buflen)
{
	...
	for (; i < end && i < descq->conf.rngsz; i++, p += desc_sz) {
		...
		if (fl) {
			len += snprintf(buf + len, buflen - len,
				" fl pg 0x%p, virt addr 0x%p, 0x%llx.\n",
				fl->pg, page_address(fl->pg), fl->dma_addr);
			fl++;
		} else
			buf[len++] = '\n';
	}
