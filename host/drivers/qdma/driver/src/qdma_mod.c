...
EXPORT_SYMBOL(xpdev_find_by_idx);
...
#endif
	if (conf.master_pf)
		conf.data_msix_qvec_max = min_t(unsigned int, 5, num_online_cpus());
	else
		conf.data_msix_qvec_max = min_t(unsigned int, 6, num_online_cpus());
