...
int main(int argc, char** argv)
{
    ...
    printf("selective_filter -> lwe_encrypt FPGA pipeline passed\n");
    printf("total_count=%u selected_count=%zu\n",
           options.record_count,
           fpga_selected_count);
    printf("fpga_filter_selected_count_derived_from_result=yes\n");
    printf("decrypted_count=%zu\n", clears.size());
    if (clears.empty()) {
        printf("decrypted_first_u8=none\n");
    } else {
    ...
    printf("\n");
    if (options.sql_mode) {
        printf("filter_protocol=sql-metadata-v2 record_bytes=%u predicates=%zu tokens=%zu\n",
               options.record_bytes, options.sql_program.predicates.size(),
               options.sql_program.tokens.size());
        printf("schema=%s\nquery=%s\n", options.schema.c_str(), options.query.c_str());
    } else {
        printf("filter_protocol=legacy-v1\n");
        printf("predicate=quantity_%s_%u quantity_offset=%u record_bytes=%u\n",
               options.predicate == kPredicateGt ? "gt" : "eq",
               options.threshold, kQuantityOffset, kRecordBytes);
    }
    printf("input_path=SSD(nsid=%u,lba=%llu,lbas=%u)->SLM->selective_filter->lwe_encrypt\n",
           options.storage_nsid,
           static_cast<unsigned long long>(options.ssd_lba),
           options.input_lbas);
    printf("ssd_copy_bytes=%zu record_stream_bytes=%zu\n",
           input_bytes,
           compute_input_bytes);
    ...
    printf("slm_read_chunk_bytes=%zu slm_read_requests=%zu "
           ...
           slm_read_request_count,
           options.slm_read_queue_depth);
    printf("filter_operator_type_id=%u lwe_operator_type_id=%u program_id=%u rsid=%u\n",
           options.filter_operator_type_id,
           options.lwe_operator_type_id,
           options.program_id,
           rsid);
