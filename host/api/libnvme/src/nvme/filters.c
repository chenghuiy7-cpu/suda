...
const char *nvme_ctrl_sysfs_dir = "/sys/class/nvmq";
const char *nvme_ns_sysfs_dir = "/sys/block";
const char *nvme_subsys_sysfs_dir = "/sys/class/nvmq-subsystem";
...
int nvme_namespace_filter(const struct dirent *d)
{
	...
	if (strstr(d->d_name, "nvmq"))
		if (sscanf(d->d_name, "nvmq%dn%d", &i, &n) == 2)
			return 1;
...
int nvme_paths_filter(const struct dirent *d)
{
	...
	if (strstr(d->d_name, "nvmq"))
		if (sscanf(d->d_name, "nvmq%dc%dn%d", &i, &c, &n) == 3)
			return 1;
...
int nvme_subsys_filter(const struct dirent *d)
{
	...
	if (strstr(d->d_name, "nvmq-subsys"))
		if (sscanf(d->d_name, "nvmq-subsys%d", &i) == 1)
			return 1;
