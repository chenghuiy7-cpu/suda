...
#define USER_APP0 3
#define USER_APP1 4
#define OPERATOR_ADD_5 0
#define OPERATOR_ADD_1 1
#define OPERATOR_ADD_3 2


enum nvme_virtual_project_type{
	BDEV,
	MEMORY,
	UNKNOWN
};
/**
 * struct nvme_virtual_project————描述用户要处理的数据
 * 
 */
struct nvme_virtual_project{
	enum nvme_virtual_project_type type;
};

/**
 * struct nvme_simple_graph_side_des————描述简单图的一条边
 * @src_port_num: 表示数据流来自源算子的哪个端口
 * @des_port_num: 表示数据流来自目的算子的哪个端口
 * @src_oper_num: 表示数据流来自哪个算子
 * @des_oper_num: 表示数据流要送到哪个算子
 * 16B
 */
struct nvme_simple_graph_side_des{
	__le16 src_port_num;
	__le16 des_port_num;
	__le16 src_oper_num;
	__le16 des_oper_num;
...
/**
 * struct nvme_simple_graph_sides_list————记录两个节点间的多条连接边
 * @side_num: 表示这个结构包含几条边
 * @side_capacity: 表示这个数据结构最多能记录几条表，固定为5
 * @side_list:	记录每条边的数组
 */
struct nvme_simple_graph_sides_list{
	__u32 side_num;
	__u32 side_capacity;
	struct nvme_simple_graph_side_des side_list[5];	
};

struct nvme_simple_graph{
	struct nvme_simple_graph_sides_list graph_structure[4][4];
};

/**
 * struct nvme_kernel_exec_spec_cmd————专门为kernel执行准备的cmd，命令结构相比于正常的NVMe命令有所变化
 * @opcode: OPC为KERNEL_EXEC
 * @priority: 请求任务的优先级
 * @virtual_object_src_num: 虚拟对象来源的数目
 * @apply_operators_name_map: 申请的算子索引和实际算子类型的对应关系，当前最多支持四个
 * @apply_operator_num: 申请的算子数目
 * @vp0_size: 虚拟对象的大小，单位为一个数据块，当前一共支持三个虚拟对象
 * @vp0_type: 虚拟对象的类型，支持内存和块设备
 * @vp_addr0: 虚拟对象包含的地址信息，如果是块设备，那就是lba，如果是内存，那就是内存地址
 * 内存地址是包含了一个数据块偏移的部分的，lba不包含！！！
 */
 struct nvme_kernel_exec_spec_cmd { 
	uint8_t		opcode :8;
	...
	uint8_t		priority:8;
	uint8_t		apply_operator_num:8;
	uint8_t		apply_operators_name_map[6];
	uint8_t	    vp_type[4];
	uint32_t	payload_size;	   /*整个cmd由一个exec spec cmd和大小为payload_size的payload组成*/
	uint32_t	vp_size[4];        /*能够操纵的vp的大小，以4KB为单位*/
	uint32_t	vppayload_num;     /*vppayload的总共大小64B*vppayload_num */
	uint64_t	software_size;     /*需要让ARM核调用的软件的大小*/
	uint32_t	software_param[4]; /*需要让ARM核调用的软件的参数设置*/
	uint32_t	timeout_ms;
	uint32_t	result;
};

/**
 * struct nvme_kernel_exec_spec_cmd_vppayload————用于存放虚拟对象列表的负载（以命令的形式）
 * @vp_addr: 虚拟对象的地址，数据块对齐
 */
struct nvme_kernel_exec_spec_cmd_vppayload{
	__u64 	vp_addr[8];
	__u32	timeout_ms;
	__u32	result;
};
...
/**
 * struct nvme_kernel_compute_request————用于描述一次计算任务的请求，包含speccmd和vppayload以及graph
 */
struct nvme_kernel_compute_request{
	struct nvme_kernel_exec_spec_cmd_vppayload vppayload[4096*2/8];
	struct nvme_kernel_exec_spec_cmd header;
	struct nvme_simple_graph graph;
	uint64_t curr_vp_index;						//vppayloadsize是以64B为单位的，而curr_vp_index以每条地址为单位
};

/**
 * struct nvme_kernel_vp————用户申请的虚拟对象的描述向量
 */
struct nvme_kernel_vp{
	__u64 addr[4096*2];
	enum nvme_virtual_project_type type;
	__u32 size;
};
...
struct nvme_passthru_kernel {
	struct nvme_passthru_cmd64 *cmds;
	__u8   num_cmds;
};

struct nvme_kernel {
	struct nvme_passthru_kernel passthru_kernel;
	__u64 result;
	int cur_memhandle;
	int fd;
	__u8 max_cmds;
...
/**
 * nvme_kernel_io() - Append an nvme user I/O command to the kernel
 * @args:	&struct nvme_io_args argument structure
 * @opcode:	Opcode to execute
 *
 * Return: The nvme command status if a response was received (see
 * &enum nvme_status_field) or -1 with errno set otherwise.
 */
int nvme_kernel_io(struct nvme_kernel *kernel, struct nvme_io_args *args, __u8 opcode);

int nvme_kernel_init(struct nvme_kernel *kernel, int fd);

int nvme_kernel_submit(struct nvme_kernel *kernel);

int nvme_kernel_mem_alloc(struct nvme_kernel *kernel, __u64 size, int *mem_handle);

int nvme_kernel_compute_exec(struct nvme_kernel* kernel,
							 struct nvme_kernel_compute_request* compute_req);

int nvme_kernel_apply_operators(struct nvme_kernel_compute_request* compute_req,
								int operator_type);

int nvme_kernel_connect_operators(struct nvme_kernel_compute_request* compute_req,
								  uint32_t src_vec,
								  uint32_t des_vec,
								  uint32_t src_port,
								  uint32_t des_port);

void nvme_kernel_create_compute_request(struct nvme_kernel_compute_request* compute_req);

int nvme_kernel_add_vp_to_compute(
	struct nvme_kernel_compute_request* compute_req,
	struct nvme_kernel_vp vp
);

struct nvme_kernel_vp nvme_kernel_create_host_mem_vp(
	void* mem_ptr,
	uint32_t size
);

struct nvme_kernel_vp nvme_kernel_create_dev_mem_vp(
	void* mem_ptr,
	uint32_t size,
	uint32_t offset
);
...
/**
 * nvme_kernel_read() - Append an nvme user read command to kernel
 * @args:	&struct nvme_io_args argument structure
 *
 * Return: The nvme command status if a response was received (see
 * &enum nvme_status_field) or -1 with errno set otherwise.
 */
static inline int nvme_kernel_read(struct nvme_kernel *kernel, struct nvme_io_args *args)
{
	return nvme_kernel_io(kernel, args, nvme_cmd_read);
}

/**
 * nvme_kernel_mem_copy() - Append an nvme user memory copy command to kernel
 * It copies from the memory region pointed to by args->data to args->slba
 * @args:	&struct nvme_io_args argument structure
 *
 * Return: The nvme command status if a response was received (see
 * &enum nvme_status_field) or -1 with errno set otherwise.
 */
static inline int nvme_kernel_mem_copy(struct nvme_kernel *kernel, struct nvme_io_args *args)
{
	return nvme_kernel_io(kernel, args, nvme_cmd_mem_copy);
}

static inline int nvme_kernel_exec(struct nvme_kernel *kernel, struct nvme_io_args *args, unsigned char opcode)
{
	return nvme_kernel_io(kernel, args, opcode);
}
...
/**
 * nvme_kernel_write() - Append an nvme user write command to kernel
 * @args:	&struct nvme_io_args argument structure
 *
 * Return: The nvme command status if a response was received (see
 * &enum nvme_status_field) or -1 with errno set otherwise.
 */
static inline int nvme_kernel_write(struct nvme_kernel *kernel, struct nvme_io_args *args)
{
	return nvme_kernel_io(kernel, args, nvme_cmd_write);
}
...
/**
 * nvme_create_slm_ns - Create NVMe SLM Namespace
 * Does not support in normal NVMe SLM Spec
 * But support in our system :)
 * 
 */
int nvme_create_slm_ns(
	int fd,
	unsigned int* nsid,
	unsigned long long nsze //Bytes
);

/**
 * nvme_delete_slm_ns - Delete NVMe SLM Namespace
 * Does not support in normal NVMe SLM Spec
 * But support in our system :)
 * 
 */
int nvme_delete_slm_ns(
	int fd,
	unsigned int nsid
);
...
int nvme_create_memory_range_set(
	int fd,
	unsigned int nsid,
	unsigned int* rsid,
	int numr,
 ...
 */
int nvme_delete_memory_range_set(
	int fd,
	unsigned int nsid,
	unsigned int rsid
);
...
/**
 * nvme_load_one_program
 * //当前的设计仅支持一次传输完成所有的程序
 * //标准规范支持一个程序通过发送多次命令完成传输
 */
static inline int nvme_operate_program(
	int fd,
	int nsid,
	unsigned int pind,
	int ptype,
	...
	struct nvme_passthru_cmd cmd;
	cmd.nsid = nsid;
	cmd.opcode = nvme_admin_load_program;
	cmd.cdw10 = (pit << 25) | (sel << 24) | (ptype << 16) | (pind);
	cmd.data_len = numb;
	cmd.addr = (unsigned long long)(program_data);
	cmd.metadata_len = 0;
	cmd.flags = 0;
	if(pit==1){
		cmd.cdw13 = pid;
	...
	//This field shall be an integer multiple of dwords
	//i.e bits 01:00 cleared to 00b
	cmd.cdw11 = psize;
	cmd.cdw14 = numb;
	cmd.cdw15 = load_offset;
 ...
 */
int nvme_unload_hlsacc_program(
	int fd,
	int pind,
	int nsid
);
 ...
 * nvme_program_mgmt
 */
static inline int nvme_program_mgmt(
	int fd,
	int pind,
	int sel,
	int nsid
 ...
 */
static inline int nvme_execute_program(
	int fd,
	int nsid,
	int rsid,
	int pind,
	...
	int dlen,
	void* data_buffer,
	unsigned long long capram1,
	unsigned long long capram2,
	unsigned long long capram3,
	unsigned int *result
){
	//内存集前N个用于输入后M个用于输出
	struct nvme_passthru_cmd cmd;
	cmd.addr = (unsigned long long)data_buffer;
	cmd.nsid = nsid;
	cmd.opcode = 1;
	cmd.data_len = dlen;
	cmd.cdw2 = rsid << 16 | pind;
	cmd.cdw3 = numr;
	cmd.flags = 0;
	cmd.metadata_len = 0;
 ...
 */
int nvme_execute_hlsacc_program(
	int fd,
	int nsid,
	int rsid,
	int pind,
	struct AccContext* contexts,
	int context_num,
	int numr,
	int priority,
	unsigned int* result
...
int nvme_execute_hlsacc_programV2(
	int fd,
	int nsid,
	int rsid,
	int pind,
...
/**
 * nvme_slm_read
 * Read Data From the specified namespace
 * @data: where data is transferred to
 * @starting_bytes: the first byte in the specified namespace to read
 * @read_length: the read length in bytes (dward granular)
 * 
 */
int nvme_slm_read(
	int fd,
...
/**
 * nvme_slm_write
 * writes data to the specified namespace
 * @data: where data is transferred from
 * @starting_bytes: the first byte in the specified namespace to write
 * @write_length: write_length in bytes
 */
int nvme_slm_write(
	int fd,
	int nsid,
...
/**
 * nvme_slm_copy
 * Used by the host to copy data from one or more 
 * source ranges in one or more source namespaces to
 * a single consecutive address range in a 
 * destination memory namespace
 * if you want to copy memory data to block device
 * you should use nvme_copy in nvm namespace 
 * @saddr: Start Destination Address
 * @format_sel: Descriptor Format Type 02h LBA Based 04h Bytes Based
...
/**
 * nvme_slm_fill
 * Fills a memory range of the specified namespace with zeros
 */
int nvme_slm_fill(
	int fd,
	int nsid,
