...
int nvme_kernel_io(struct nvme_kernel *kernel, struct nvme_io_args *args, __u8 opcode)
{
	...
	const size_t size_v2 = sizeof_args(struct nvme_io_args, pif, __u64);
	__u32 cdw2, cdw3, cdw10, cdw11, cdw12, cdw13, cdw14, cdw15;
	struct nvme_passthru_cmd64 *cmd = &kernel->passthru_kernel.cmds[kernel->passthru_kernel.num_cmds];
	...
	cmd->cdw14 = cdw14;
	cmd->cdw15 = cdw15;
	cmd->data_len = args->data_len;
	cmd->metadata_len = args->metadata_len;
	cmd->addr = (__u64)(uintptr_t)args->data;
	cmd->metadata = (__u64)(uintptr_t)args->metadata;
	cmd->timeout_ms = args->timeout;

	kernel->passthru_kernel.num_cmds++;
	...
	if (kernel->fd == -1) {
		kernel->fd = args->fd;
	} else if (kernel->fd != args->fd) {
		fprintf(stderr, "Inconsistent fd: %d vs %d\n", kernel->fd, args->fd);
		errno = EINVAL;
		return -1;
	}
...
int nvme_kernel_init(struct nvme_kernel *kernel, int fd)
{
	...
	kernel->max_cmds = 64;
	kernel->passthru_kernel.num_cmds = 0;
	kernel->cur_memhandle = 0;
	kernel->fd = fd;
	kernel->passthru_kernel.cmds = calloc(kernel->max_cmds, sizeof(struct nvme_passthru_cmd64));

	memset(&args, 0, sizeof(args));
	args.args_size = sizeof(args);
	args.fd = fd;
	return nvme_kernel_io(kernel, &args, nvme_cmd_kernel_start);
}

int nvme_kernel_submit(struct nvme_kernel *kernel)
{
	struct nvme_passthru_cmd64 *fake_cmd = (struct nvme_passthru_cmd64 *)kernel;
	struct nvme_io_args args;
	int ret;
	...
	args.args_size = sizeof(args);
	args.fd = kernel->fd;
	ret = nvme_kernel_io(kernel, &args, nvme_cmd_kernel_end);
	if (ret) {
		return ret;
	}
	return nvme_submit_passthru64(kernel->fd, NVME_IOCTL_KERNEL, fake_cmd, &kernel->result);
}
...
int nvme_kernel_mem_alloc(struct nvme_kernel *kernel, __u64 size, int *mem_handle)
{
	...
	memset(&args, 0, sizeof(args));
	// WARNING: u64 to u32 conversion!
	args.data_len = size;
	args.slba = kernel->cur_memhandle;
	args.args_size = sizeof(args);
	args.fd = kernel->fd;
	
	*mem_handle = kernel->cur_memhandle++;

	return nvme_kernel_io(kernel, &args, nvme_cmd_mem_alloc);
}

void nvme_kernel_create_compute_request(struct nvme_kernel_compute_request* compute_req){
	//compute_req = malloc(sizeof(struct nvme_kernel_compute_request));
	memset(compute_req,0,sizeof(struct nvme_kernel_compute_request));
	compute_req->header.opcode = nvmd_cmd_compute_exec;
}

int nvme_kernel_apply_operators(struct nvme_kernel_compute_request* compute_req,int operator_type){
	if(compute_req->header.apply_operator_num > 4)
		return -1;
	else{
		compute_req->header.apply_operators_name_map[compute_req->header.apply_operator_num] = operator_type;
		return compute_req->header.apply_operator_num++;
	}
}


int nvme_kernel_connect_operators(struct nvme_kernel_compute_request* compute_req,uint32_t src_vec,uint32_t des_vec,uint32_t src_port,uint32_t des_port){	
	struct nvme_simple_graph_side_des side;
	if(src_vec >= 5 || des_vec >= 5)
		return -1;
	side.des_oper_num = des_vec;
	side.src_oper_num = src_vec;
	side.des_port_num = des_port;
	side.src_port_num = src_port;
	compute_req->graph.graph_structure[src_vec][des_vec].side_list[compute_req->graph.graph_structure[src_vec][des_vec].side_num] = side;
	compute_req->graph.graph_structure[src_vec][des_vec].side_num += 1;
    return 0;
}
...
struct nvme_kernel_vp nvme_kernel_create_file_vp(
	char* path,
	uint32_t size
){
	struct nvme_kernel_vp vp;
	...
	//如果文件的大小和用户申请的大小不同，用户申请的比较大
	//那么需要扩展文件大小到用户申请的大小

	//然后使用fiemap将块设备的映射信息返回
	//暂时是TODO！！！
	/***************************** 
	 * 			TODO		     *
	******************************/
...
int nvme_kernel_add_vp_to_compute(
	...
	if(vp.type == MEMORY){
		compute_req->vppayload[compute_req->header.vppayload_num].vp_addr[compute_req->curr_vp_index++] = vp.addr[0];
		compute_req->header.vp_type[compute_req->header.vppayload_num] = MEMORY;
		compute_req->header.vp_size[compute_req->header.vppayload_num] = vp.size / 4096; //TODO，这里要设置大小以页为单位，以后它或许会成为BUG
		if(compute_req->curr_vp_index > 8)
		{
			compute_req->header.vppayload_num++;
			compute_req->curr_vp_index = 0;
		}
		compute_req->vppayload[compute_req->header.vppayload_num].vp_addr[compute_req->curr_vp_index++] = vp.addr[1];
...
int nvme_kernel_compute_exec(
							 struct nvme_kernel* kernel,
							 struct nvme_kernel_compute_request* compute_req
							)
{
	if(compute_req->curr_vp_index != 0)
		compute_req->header.vppayload_num += 1;

	compute_req->header.payload_size = 1 + compute_req->header.vppayload_num + sizeof(struct nvme_simple_graph)/64;
	uint32_t size;
	size = compute_req->header.vppayload_num;
	struct nvme_passthru_cmd64* cmd = &kernel->passthru_kernel.cmds[kernel->passthru_kernel.num_cmds];
	kernel->passthru_kernel.num_cmds ++;
	memcpy(cmd,(struct nvme_passthru_cmd64*)&compute_req->header,sizeof(struct nvme_passthru_cmd64));
	unsigned int index = 0;
	while(size>0)
	{
		cmd = &kernel->passthru_kernel.cmds[kernel->passthru_kernel.num_cmds];
		memcpy(cmd,(struct nvme_passthru_cmd64*)(compute_req->vppayload + index),sizeof(struct nvme_passthru_cmd64));
		size -= 1;
		kernel->passthru_kernel.num_cmds ++;
		for(int i=0; i < 8;i++){
			printf("PRP ADDR %x\n",compute_req->vppayload[index].vp_addr[i]);
		}
	}
	size = sizeof(struct nvme_simple_graph)/64;
	uint32_t graph_index = 0;
	while(size != 0){
		cmd = &kernel->passthru_kernel.cmds[kernel->passthru_kernel.num_cmds];
		memcpy(cmd,(void*)(&(compute_req->graph))+(graph_index*64),64);
		kernel->passthru_kernel.num_cmds ++;
		size --;
		graph_index ++;
	}
...
int nvme_copy(struct nvme_copy_args *args)
{
	...
	if (args->format == 1)
		...
		data_len = args->nr * sizeof(struct nvme_mc_source_range);
	else
...
{
	//struct nvme_ns_mgmt_host_sw_specified raw_data;
	struct nvme_ns_mgmt_host_sw_specified* data = (struct nvme_ns_mgmt_host_sw_specified*)calloc(1,2048);
	//raw_data.nsze = nsze;
	data->nsze = nsze;
	...
	struct nvme_passthru_cmd cmd = {
		...
		.cdw10	    = cdw10,
		.cdw11	    = cdw11,
		.cdw12		= nsze,		//当前仅通过nsze传递数据，所以不需要data
		.timeout_ms = args.timeout,
	};

	if (args.args_size == size_v2) {
		if (args.data) {
			cmd.data_len = 768;//sizeof(struct nvme_ns_mgmt_host_sw_specified);
			cmd.addr = (__u64)(uintptr_t)args.data;
			//printf("ok i get data\n");
		}
	}
	else {
		if (args.ns) {
...
int nvme_delete_slm_ns(
	...
	struct nvme_passthru_cmd cmd = {
		...
		.cdw10	    = cdw10,
		.cdw11	    = cdw11,
		.cdw12		= 0,		//当前仅通过nsze传递数据，所以不需要data
		.timeout_ms = args.timeout,
	};
	cmd.data_len = 0;
	cmd.addr = (__u64)NULL;
...
int nvme_create_memory_range_set(
	...
	return nvme_operate_memory_range_set(fd,nsid,0,numr,rsid,mmrange_descri);
}
...
int nvme_delete_memory_range_set(
	...
	return nvme_operate_memory_range_set(fd,nsid,1,0,&rsid,NULL);
}
...
int nvme_execute_hlsacc_program(
	...
	unsigned long long capram1 = 0;
	for(int i=0;i<context_num;i++){
		capram1 = ((capram1 << 8)|1);
	}
	return nvme_execute_program(fd,nsid,rsid,pind,numr,
	4096*context_num,(void*)contexts,
	capram1,priority,0,result);
}
...
int nvme_execute_hlsacc_programV2(
	...
	return nvme_execute_program(fd,nsid,rsid,pind,numr,
	4096*context_num,(void*)contexts,
	capram1,priority,run_way,result);
}
...
int nvme_load_hlsacc_program(
	...
	//Pid is not used
	int numb = psize>4096? 4096:psize;
	int offset = 0;
	int ret = nvme_operate_program(fd,nsid,pind,fusion_program,psize,numb,0,0,0,program_data,0);
	offset += numb;
	if(ret!=0) return ret;
	while(offset<psize){
		numb = (psize-offset)>4096?4096:(psize-offset);
		ret = nvme_operate_program(fd,nsid,pind,fusion_program,psize,numb,offset,0,0,program_data+offset,0);
		offset += numb;
		if(ret!=0) return ret;
	}
	return 0;
...
int nvme_unload_hlsacc_program(
	...
	return nvme_operate_program(fd,nsid,pind,fusion_program,0,0,0,0,1,NULL,0);
}
...
int nvme_activate_program(
	...
	return nvme_program_mgmt(fd,pind,0,nsid);
}
...
int nvme_deactivate_program(
	...
	return nvme_program_mgmt(fd,pind,1,nsid);
}
...
int nvme_slm_read(
	...
	return nvme_io_passthru(
		fd,0x02,0,0,nsid,0,0,starting_bytes,0,
...
int nvme_slm_write(
	...
	//opcode 05h
	return nvme_io_passthru(
		fd,0x05,0,0,nsid,0,0,starting_bytes,0,
		write_length,0,0,0,write_length,data,0,NULL,0,NULL
	);
}
...
int nvme_slm_copy(
	...
	//opcode 01h
	return nvme_io_passthru(fd,0x01,0,0,nsid,length,length>>32,sdaddr,sdaddr>>32,
	format_sel<<8|(nr-1),0,0,0,4096,(void*)source_range_entries,0,NULL,0,
	NULL);
}
...
int nvme_slm_fill(
	...
	return nvme_io_passthru(fd,0x00,0,0,nsid,0,0,starting_bytes,starting_bytes>>32,
	fill_length,0,0,0,0,NULL,0,NULL,0,NULL);
}