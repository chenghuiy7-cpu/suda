...
enum nvme_admin_opcode {
	...
	nvme_admin_prog_mgmt		= 0x88,
	nvme_mmrange_set_mgmt		= 0x89,
};
...
enum nvme_io_opcode {
	...
	nvme_cmd_kernel_start	= 0x20,
	nvme_cmd_mem_alloc	= 0x21,
	nvme_cmd_mem_copy	= 0x22,
	nvme_cmd_compute_op1	= 0x23,
	nvme_cmd_compute_op2	= 0x24,
	nvme_cmd_compute_op3	= 0x25,
	nvmd_cmd_compute_exec  = 0x26,
	nvme_cmd_kernel_end	= 0x30,
...
union nvme_source_range{
	struct nvme_mc_source_range mc;
	struct nvme_scc_source_range scc;
};
...
struct memory_range_descriptor{
	...
	unsigned int resvd1;
	unsigned long long resvd2;
};

union memory_range_set_decriptor{
	struct{
		unsigned int rsid:2;
		unsigned int nmr:4; //Number of Memory Range Descritpors
		unsigned int rsvd:16;
	}header;
	struct memory_range_descriptor payload;
};
...
union AssSchedCmd{
    struct{
        ...
        enum AssSchedCmdOpcode opc:8;              //1B
...
//4+22*(8)+6*8+4+64+1+1+1+64+4+4+8+4
struct hlsacccompute_program{
    ...
    uint32_t program_id;
    uint8_t apply_operators_id_map[8]; // 算子逻辑号到算子类型的映射
    uint8_t input_channel_destination[8];
...
union nvme_downable_prog_type_list{
	struct{
		unsigned int numd;
