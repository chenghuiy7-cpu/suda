...
int nvme_scan_topology(struct nvme_root *r, nvme_scan_filter_t f, void *f_args)
{
	...
	if (subsys.num < 0) {
		nvme_msg(r, LOG_WARNING, "failed to scan subsystems: %s\n",
			 strerror(errno));
		return subsys.num;
	}

	printf("subsys.num is %d\n", subsys.num);
...
static void nvme_subsystem_set_path_ns(nvme_subsystem_t s, nvme_path_t p)
{
	...
	ret = sscanf(nvme_path_get_name(p), "nvmq%dc%dn%d", &i, &c, &nsid);
	if (ret != 3)
		return;

	sprintf(n_name, "nvmq%dn%d", i, nsid);
...
int nvme_init_ctrl(nvme_host_t h, nvme_ctrl_t c, int instance)
{
	...
	ret = asprintf(&name, "nvmq%d", instance);
	if (ret < 0) {
		errno = ENOMEM;
		return -1;
	}
	ret = asprintf(&path, "%s/nvmq%d", nvme_ctrl_sysfs_dir, instance);
	if (ret < 0) {
		errno = ENOMEM;
		return ret;
	}
...
static void nvme_ns_set_generic_name(struct nvme_ns *n, const char *name)
{
	...
	ret = sscanf(name, "nvmq%dn%d", &instance, &head_instance);
	if (ret != 2)
		return;
...
static char *nvme_ns_generic_to_blkdev(const char *generic)
{
	...
	sprintf(blkdev, "nvmq%dn%d", instance, head_instance);
...
static void nvme_subsystem_set_ns_path(nvme_subsystem_t s, nvme_ns_t n)
{
	...
	ret = sscanf(nvme_ns_get_name(n), "nvmq%dn%d", &ns_ctrl, &ns_nsid);
	if (ret != 2)
		return;
	...
	nvme_subsystem_for_each_ctrl(s, c) {
		nvme_ctrl_for_each_path(c, p) {
			...
			ret = sscanf(nvme_path_get_name(p), "nvmq%dc%dn%d",
