...
impl HpuHw {
    ...
    pub fn new_hpu_hw(
        ...
        // Try current hw and fallback to a fresh reload, unless reload is explicitly disabled.
        if force == "never" {
            return Self::try_current_hw(id, &hpu_pdi, ami_retry, h2c_path, c2h_path)
                .unwrap_or_else(|err| {
                    panic!(
                        "Loading current HW failed with {err:?}; fresh reload disabled by force_reload=never"
                    )
                });
        }
