...
impl HpuRadixCiphertext {
    ...
    /// Create an HPU radix ciphertext from coefficients that are already in
    /// the native per-PC HPU order.
    ///
    /// Unlike [`Self::from_radix_ciphertext`], this constructor performs no
    /// bit reversal, coefficient interleaving, or torus-width conversion.
    /// The caller must provide one native HPU LWE entity per radix block.
    pub fn from_hpu_lwe_ciphertexts(
        hpu_ct: Vec<HpuLweCiphertextOwned<u64>>,
        device: &HpuDevice,
    ) -> Self {
        assert!(
            !hpu_ct.is_empty(),
            "cannot create an empty HPU radix ciphertext"
        );
        assert!(
            hpu_ct.iter().all(|ct| ct.params() == device.params()),
            "native HPU ciphertext parameters do not match the target device"
        );
        Self(device.new_var_from(hpu_ct, VarMode::Native))
    }
    ...
    /// Return the synchronized ciphertext in the native per-PC HPU layout.
    ///
    /// This avoids the CPU-LWE conversion performed by
    /// [`Self::to_radix_ciphertext`] and is intended for device-to-device
    /// pipelines that preserve the HPU physical coefficient order.
    pub fn to_hpu_lwe_ciphertexts(&self) -> Vec<HpuLweCiphertextOwned<u64>> {
        self.0.clone().into_ct()
    }
